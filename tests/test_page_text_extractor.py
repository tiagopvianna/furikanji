...
class TestPageTextExtractor(unittest.TestCase):
    def test_requires_adapters_when_ocr_is_enabled(self):
        with self.assertRaises(ValueError):
            PageTextExtractor(disable_ocr=False)
    ...
    def test_split_line_image_for_transcription_does_not_split_small_ratio(self):
        extractor = PageTextExtractor(disable_ocr=True)
        line_image = np.ones((16, 10, 3), dtype=np.uint8)
        line_text_mask = np.ones((16, 10), dtype=np.uint8)

        line_crops, split_points = extractor._split_line_image_for_transcription(
        ...
        self.assertEqual(len(line_crops), 1)
        self.assertEqual(split_points, [])
    ...
    def test_split_line_image_for_transcription_splits_wide_line(self):
        ...
        self.assertGreater(len(line_crops), 1)
